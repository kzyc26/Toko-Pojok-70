document.querySelector('.user').style.display = 'none';
document.querySelector('.accumulated').style.display = 'none';
document.querySelector('.customerreview').style.display = 'none';
document.querySelector('.customer_subtotal').style.display = 'none';
document.querySelector('.productdetails').style.display = 'none';
document.querySelector('.itemcategory').style.display = 'none';
document.querySelector('.cartdetails').style.display = 'none';
document.querySelector('.Wishlist').style.display = 'none';
document.querySelector('.vouchercustomer').style.display = 'none';
document.querySelector('.orderhistory').style.display = 'none';
document.querySelector('.alamat').style.display = 'none';
document.querySelector('.biodata').style.display = 'none';
document.querySelector('.oldpassword').style.display = 'none';
document.querySelector('.userdelivdetails').style.display = 'none';
document.querySelector('.bestseller').style.display = 'none';
document.querySelector('.Discount').style.display = 'none';
// document.querySelector('.hasil_search').style.display = 'none';

function showuser() {
    document.querySelector('.user').style.display = 'block';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showaccumulated() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'block';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showreview() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'block';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showcustsubtotal() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'block';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showproductdetails() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'block';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showitemscategory() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'block';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showcartdetails() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'block';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};



function showwishlist() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'block';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showvoucher() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'block';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showorderhistory() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'block';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showalamat() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'block';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showbiodata() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'block';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showoldpass() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'block';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showuserdeliverydetails() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'block';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showbestseller() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'block';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showdiscount() {
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'block';
    document.querySelector('.hasil_search').style.display = 'none';
};

function showsearchresult(){
    document.querySelector('.user').style.display = 'none';
    document.querySelector('.accumulated').style.display = 'none';
    document.querySelector('.customerreview').style.display = 'none';
    document.querySelector('.customer_subtotal').style.display = 'none';
    document.querySelector('.productdetails').style.display = 'none';
    document.querySelector('.itemcategory').style.display = 'none';
    document.querySelector('.cartdetails').style.display = 'none';
    
    document.querySelector('.Wishlist').style.display = 'none';
    document.querySelector('.vouchercustomer').style.display = 'none';
    document.querySelector('.orderhistory').style.display = 'none';
    document.querySelector('.alamat').style.display = 'none';
    document.querySelector('.biodata').style.display = 'none';
    document.querySelector('.oldpassword').style.display = 'none';
    document.querySelector('.userdelivdetails').style.display = 'none';
    document.querySelector('.bestseller').style.display = 'none';
    document.querySelector('.Discount').style.display = 'none';
    document.querySelector('.hasil_search').style.display = 'block';
}

function categorychange(){    
    var cbcat = document.getElementById("listcategory");
    var cat = cbcat.options[cbcat.selectedIndex].value;
    // alert(cat);
            $.ajax({
                url: "item_category.php",
                method: "GET",
                data: { cat },
                dataType: "html",
                success: function(result){
                    $("#categoryfill").html(result);
                }
            });
}


// var query;            
//         $(document).ready(function (){
//             $('.isi').click(function(event){
//                 event.preventDefault();
//                 query = $(this).data('query');
//                 var array_judul;
//                 if (query == "SELECT username, password FROM customer"){
//                     array_judul = ["Username", "Password"];
//                 }
//                 $.ajax({
//                     url: "hasil-ajax-backend.php",
//                     method: "GET",
//                     data: { query, array_judul },
//                     dataType: "html",
//                     success: function (result){
//                         $("#hasil").html(result);
//                     }
//                 });
//             });
            
//         });